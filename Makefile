...
dwpal_cli_ldflags := -L./ -ldwpal -ldl -lcurses -lreadline -lrt -L$(STAGING_DIR)/usr/sbin/ -lsafec-1.0 -lpthread
dwpal_cli_cflags  := -I./include -I$(IWLWAV_HOSTAP_DIR)/src/common/ -I$(IWLWAV_HOSTAP_DIR)/src/utils/ -DCONFIG_CTRL_IFACE -DCONFIG_CTRL_IFACE_UNIX -I$(STAGING_DIR)/usr/include/ -I$(IWLWAV_HOSTAP_DIR)/src/drivers/
